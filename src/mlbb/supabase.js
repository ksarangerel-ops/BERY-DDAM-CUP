...
const LEGACY_TOURNAMENT_ID = 'ddam-cup-additional-games-v1';
...
export function subscribe(onNext, onError) {
  ...
  const loadCurrent = async () => {
    ...
    let snapshot = normaliseRow(data);
    if (!snapshot) {
      const { data: legacy, error: legacyError } = await client
        .from('tournaments')
        .select('state')
        .eq('id', LEGACY_TOURNAMENT_ID)
        .maybeSingle();
      if (legacyError) console.warn('[mlbb] legacy state read failed:', legacyError);
      snapshot = legacy?.state || null;
    }
    onNext(snapshot);
  };
...
/** Store a compressed Mobile Legends asset and return its public URL. */
