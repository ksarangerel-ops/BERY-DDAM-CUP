/* ============================================================
   DDAM ESPORT CUP MOBILE LEGENDS — entry point
============================================================ */
import './style.css';
  ...
  $('formatRules').innerHTML = `
    <div class="rounded-2xl glass p-5 border border-cyan/20">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div class="text-[10px] uppercase tracking-[.3em] font-display font-black text-cyan">MOBILE LEGENDS TOURNAMENT FORMAT</div>
...
function renderRosters() {
  ...
  $('mvpCard').innerHTML = champion
    ...
    : `<div class="h-full rounded-2xl border border-cyan/40 bg-gradient-to-br from-cyan/10 via-panel/80 to-ink/90 backdrop-blur-md p-5 flex flex-col justify-center text-center">
        ...
        <div class="text-[10px] uppercase tracking-[.3em] font-display font-black text-cyan">MOBILE LEGENDS CUP STATUS</div>
...
async function uploadPlayerPhoto(pid) {
  ...
  if (storedOnline) {
    try { value = await uploadMedia(`mlbb/${TOURNAMENT_ID}/players/${pid}`, data); }
    catch (error) { console.error('[mlbb] player photo upload failed:', error); toast(`Photo upload failed — ${error.message || 'Storage rejected the file'}`, true); return; }
  }
  const result = await saveRoster(next => {
...
async function uploadTeamLogo(teamId) {
  ...
  const storedOnline = isLive();
  if (storedOnline) {
    try { value = await uploadMedia(`mlbb/${TOURNAMENT_ID}/teams/${teamId}`, data); }
    catch (error) { console.error('[mlbb] team logo upload failed:', error); toast(`Logo upload failed — ${error.message || 'Storage rejected the file'}`, true); return; }
  }
  const result = await saveRoster(next => {
...
$('btnClearAll').onclick = () => clearAllResults(`Clear every Mobile Legends series result across all ${NUM_MATCHES} games?\n\nTeam names, tags, zone assignments and player names are kept.`);
$('btnReset').onclick = () => clearAllResults(`Reset every Mobile Legends series result across all ${NUM_MATCHES} games?\n\nTeam names, tags, zone assignments and player names are kept.`);
...
$('btnPng').onclick = async () => {
  ...
  try {
    ...
    link.download = `ddam-cup-mobile-legends-${new Date().toISOString().slice(0, 10)}.png`;
...
$('btnCopy').onclick = async () => {
  ...
  const text = ['🏆 DDAM ESPORT CUP — MOBILE LEGENDS', '', 'FORMAT: 2 ZONES × 3 TEAMS · ZONE BO2 → A1 vs B2 / B1 vs A2 → BO3 PLACEMENT FINALS', '']
