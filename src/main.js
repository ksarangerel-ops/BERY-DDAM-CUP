...
function renderBracket() {
  ...
  $('bracket').innerHTML = `<div class="rounded-2xl glass overflow-hidden border border-gold/25">
    ...
    <div class="grid items-start gap-4 lg:grid-cols-3 p-4 sm:p-5">
      <div class="rounded-xl border border-gold/30 bg-gold/5 p-3"><div class="text-[10px] uppercase tracking-[.2em] font-display font-black text-gold mb-3">Upper · waiting slots</div><div class="grid gap-2"><div class="bracket-match"><span>Seed 1 · ${groupRows[0] ? esc(groupRows[0].team.name) : 'Waiting'}</span><span>Upper slot</span></div><div class="bracket-match"><span>Seed 2 · ${groupRows[1] ? esc(groupRows[1].team.name) : 'Waiting'}</span><span>Upper slot</span></div></div><div class="qualifier-visual mt-4"><div class="qualifier-scan"></div><div class="qualifier-orbit"><img src="/cs2-logo.svg" alt="CS2 playoff" class="qualifier-logo"></div><div class="qualifier-kicker">QUALIFICATION CORE</div><div class="qualifier-status">${visualStatus}</div><div class="qualifier-stats"><div><strong>${groupPlayed}/15</strong><span>GROUP</span></div><div><strong>${lowerPlayed}/2</strong><span>LOWER</span></div><div><strong>${finalPlayed}/4</strong><span>PLAYOFF</span></div></div><div class="qualifier-track"><span class="${groupPlayed === 15 ? 'is-live' : ''}">01 GROUP</span><span class="${lowerPlayed === 2 ? 'is-live' : ''}">02 LOWER</span><span class="${finalPlayed === 4 ? 'is-live' : ''}">03 PODIUM</span></div></div></div>
