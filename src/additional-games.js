...
function renderPublic() { const game = state.games[activeGame]; const body = activeGame === 'mlbb' ? renderMlbbDashboard(game) : activeGame === 'mecha' ? renderMechaDashboard(game) : activeGame === 'stumble' ? renderStumbleDashboard(game) : activeGame === 'pubg' ? renderPubgDashboard(game) : activeGame === 'tetris' ? renderTetrisDashboard(game) : renderTekken(game); $('publicBoard').innerHTML = body; }
